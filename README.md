...
Encoding (encoder.predict): The image (resized to $384 \times 384$) is passed through the Encoder part of the Neural Network. This turns the human-readable image into a massive, highly complex data tensor ($\text{384} \times \text{384} \times \text{512}$ floating-point numbers), which looks like noise.

Encryption (aes_encrypt): This data tensor is converted into raw bytes and cryptographically encrypted using AES-256 with a password-derived key. The final encrypted file (encrypted_image.bin) contains the necessary salt, metadata, and ciphertext.

Decryption & Reconstruction: The encrypted data is decrypted back into the raw tensor, which is then fed through the Decoder part of the Neural Network to reconstruct the original image.
...
Install everything using the -r flag:
